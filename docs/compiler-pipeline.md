...
The Rusty compiler processes source code through six main stages, orchestrated by [`Main.kt`](src/main/kotlin/rusty/Main.kt:25):
...
6. **IR Optimization** - Runs the LLVM pass sequence `cfg-simplify -> mem2reg`

## Compilation Modes

The compiler supports six compilation modes ([`CompileMode`](src/main/kotlin/rusty/core/Constants.kt:3)):
...
- **OPT** (`opt`, `optimize`, `optimization`) - Generates optimized LLVM IR (.ll) output
