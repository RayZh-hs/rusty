...
@Peekable @Parsable
data class SelfParamNode(val isReference: Boolean, val isMutable: Boolean, val type: TypeNode?, override val pointer: CompilerPointer) : ASTNode(pointer) {
    companion object
}

@Peekable @Parsable
sealed class FunctionParamNode(pointer: CompilerPointer) : ASTNode(pointer) {
    companion object;
    
    @Peekable @Parsable data class FunctionParamTypedPatternNode(val pattern: PatternNode, val type: TypeNode?, override val pointer: CompilerPointer) : FunctionParamNode(pointer)
    @Peekable @Parsable data class FunctionParamTypeNode(val type: TypeNode, override val pointer: CompilerPointer) : FunctionParamNode(pointer)
...
fun parseFunctionParamNode(ctx: Context): FunctionParamNode {
    if (ctx.peekToken() == Token.O_TRIPLE_DOT) {
        ...
        return FunctionParamNode.FunctionParamWildcardNode(pointer)
    }
    return (
            ctx.tryParse("FunctionParam@Pattern") {
                ...
                FunctionParamNode.FunctionParamTypedPatternNode(pattern, type, pointer)
            } ?: run {
                val pointer = ctx.peekPointer()
                FunctionParamNode.FunctionParamTypeNode(
                    parseTypeNode(ctx), pointer
                )
            })
}